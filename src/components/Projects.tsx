...
const buildThemes = (base: Record<ProjectItem['category'], CatTheme>): Record<ProjectItem['category'], CatThemeWithAnim> => {
  ...
  const durations: Record<ProjectItem['category'], number> = {
    data: 2.4, healthcare: 1.6, nlp: 1.8, robotics: 2.2, tooling: 2.6, 'web-app': 2.0,
  }
...
const roleConfig: Record<string, { color: (d: boolean) => string; icon: IconType; textKey: string; }> = {
  independent: { color: (d) => (d ? '#ebcb8b' : '#c47d46'), icon: FaUser, textKey: 'projects.independent' },
  lead: { color: (d) => (d ? '#d08770' : '#b35a2e'), icon: FaCrown, textKey: 'projects.lead' },
  maintainer: { color: (d) => (d ? '#a3be8c' : '#36805a'), icon: FaSync, textKey: 'projects.maintainer' },
  'tech-lead': { color: (d) => (d ? '#88c0d0' : '#2a769c'), icon: FaCog, textKey: 'projects.techLead' },
}
...
const FlowNode: React.FC<{
  ct: CatTheme; hlc: { kw: string; num: string; str: string }; isDark: boolean; isLast: boolean;
  item: TP; onImageClick: (src: string, alt: string) => void; termBorder: string;
  termMuted: string; termSecondary: string;
  termText: string;
  ...
  return (
    <Flex align="start" gap={[3, 3, 4]} position="relative" py={3}>
      <Box flexShrink={0} mt="6px">
        <Box
          bg={item.featured ? ct.color : 'transparent'} border="2px solid" borderColor={item.featured ? ct.color : termBorder} borderRadius="full"
      ...
      <Box flex={1} minW={0}>
        <HStack align="center" flexWrap="wrap" gap={2} mb={1}>
          ...
          <HStack color={ct.color} gap={1}>
            ...
            <Text fontFamily="mono" fontSize="2xs" fontWeight="semibold" letterSpacing="wide" textTransform="uppercase">{ct.label}</Text>
          </HStack>
          <Text color={termBorder} fontSize="2xs">/</Text>
          <HStack gap={1}>
            <Icon as={role.icon} boxSize="9px" color={role.color(isDark)} />
            <Text color={role.color(isDark)} fontFamily="mono" fontSize="2xs" fontWeight="bold">{t(role.textKey)}</Text>
          </HStack>
          <Text color={termMuted} flexShrink={0} fontFamily="mono" fontSize="2xs" ml="auto">{fmtDate(item.date)}</Text>
        </HStack>

        <Text
          _hover={hasExpandable ? { color: ct.color } : undefined} color={termText} cursor={hasExpandable ? 'pointer' : undefined} fontSize={['sm', 'md']} fontWeight="semibold"
          lineHeight="tall" mb={1}
          onClick={hasExpandable ? () => setExpanded((p) => !p) : undefined}
          transition="color 0.15s"
        >
          {item.title}
          {item.featured && <Text as="span" color={hlc.num} fontSize="xs" ml={2}>★</Text>}
        </Text>

        {item.badge && (
          <HStack flexWrap="wrap" gap={1.5} mb={2}>
            <Text
              bg={isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)'} border={`1px solid ${ct.border}`} borderRadius="sm" color={ct.color} fontFamily="mono"
              fontSize="2xs" px={2}
              py={0.5}
            >
              {item.badge}
        ...
        <Flex align="stretch" direction={['column', 'column', hasImg ? 'row' : 'column']} gap={[3, 3, 4]}>
          {hasImg && (
            <MotionHover>
              <Box
          ...
          <VStack align="start" flex={1} gap={2.5} justify="center" minW={0}>
            ...
            <HStack flexWrap="wrap" gap={1.5}>
              {res.map((r) => (
                <MotionHover key={r.url}>
                  <Link _hover={{ textDecoration: 'none' }} href={r.url} onClick={(e) => e.stopPropagation()} rel="noopener noreferrer" target="_blank">
                    <HStack _hover={{ borderColor: ct.color, color: ct.color }} border="1px solid" borderColor={termBorder} borderRadius="sm" color={termSecondary} fontFamily="mono"
                      fontSize="xs" gap={1.5} px={2.5} py={1} transition="all 0.15s">
              ...
              {hasExpandable && (
                <MotionHover>
                  <HStack _hover={{ borderColor: ct.color, color: ct.color }} as="button" border="1px solid" borderColor={expanded ? ct.color : termBorder} borderRadius="sm" color={expanded ? ct.color : termSecondary} fontFamily="mono" fontSize="xs"
                    gap={1.5} onClick={() => setExpanded((p) => !p)} px={2.5}
                    py={1} transition="all 0.15s"
                  >
                    <Icon as={FaChevronDown} boxSize="8px" transform={expanded ? 'rotate(180deg)' : undefined} transition="transform 0.15s" />
            ...
            {item.tags.length > 0 && (
              <HStack flexWrap="wrap" gap={1.5}>
                {item.tags.map((t) => (
                  <Text bg={isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.04)'} borderRadius="sm" color={termMuted} fontFamily="mono" fontSize="2xs" key={t} px={1.5} py={0.5}>
                    {t}
                  </Text>
                ))}
              </HStack>
        ...
        <Collapsible.Root open={expanded}>
          <Collapsible.Content>
            <VStack align="stretch" gap={3} mt={3}>
              ...
              {item.story && (
                <Box bg={isDark ? 'rgba(0,0,0,0.15)' : 'rgba(0,0,0,0.02)'} borderLeft="2px solid" borderLeftColor={ct.color} borderRadius="md" p={3}>
                  <Text color={termMuted} fontSize="xs" fontStyle="italic" lineHeight="tall">"{highlightData(item.story, hlc)}"</Text>
                </Box>
              )}
            </VStack>
          </Collapsible.Content>
  ...
  return (
    <Box py={8} w="full">
      <VStack gap={4} maxW="1400px" mx="auto" px={[2, 4, 8]}>
        <Box border="1px solid" borderColor={termBorder} borderRadius="md" boxShadow="lg" fontFamily="mono" overflow="hidden" w="full">
          ...
          <Flex align="center" bg={termHeader} color={termText} fontSize="xs" justify="space-between" px={4} py={2}>
            <HStack gap={3}>
              <HStack gap={1.5}>
            ...
            <Text color={termHighlight} fontSize="xs">
              {new Date().toLocaleTimeString('en-US', { hour: '2-digit', hour12: false, minute: '2-digit', second: '2-digit' })}
            </Text>
          </Flex>

          <Flex align="center" bg={tc.touchBar} borderBottom={`1px solid ${termBorder}`} fontSize="2xs" justify="space-between" overflow="hidden" px={4} py={1}>
          ...
          <Flex bg={termTabBar} borderBottom={`1px solid ${termBorder}`} css={{ '&::-webkit-scrollbar': { height: '0' } }} overflowX="auto">
            {tabs.map((tab) => {
          ...
          <Flex align="center" bg={termBg} borderBottom={`1px solid ${termBorder}`} fontSize="xs" gap={2} px={4} py={2}>
            <Text color={termPrompt} flexShrink={0}>{siteOwner.terminalUsername}@projects:{promptPath}$</Text>
            <Input _focus={{ outline: "none" }} _placeholder={{ color: termSecondary }} border="none" color={termText} flex="1" fontFamily="mono" minW="120px" onChange={(e) => setSearchQuery(e.target.value)} outline="none" placeholder="grep -i '...'" size="xs" value={searchQuery} />
          </Flex>

          <Box bg={termBg} color={termText} css={{ '&::-webkit-scrollbar': { background: 'transparent', width: '6px' }, '&::-webkit-scrollbar-thumb': { background: tc.border, borderRadius: '3px' } }} key={activeTab} maxH="75vh" overflowY="auto">
            <Box px={[3, 4, 5]} py={4}>
                ...
                {yearGroups.map((group, gi) => (
                  <Box key={group.year} mb={gi < yearGroups.length - 1 ? 6 : 0}>
                    <HStack gap={2} mb={2} pl="2px">
                      <Text color={termHighlight} fontFamily="mono" fontSize="2xs" fontWeight="semibold" letterSpacing="wide">{group.year}</Text>
                      <Box bg={termBorder} flex="1" h="1px" opacity={0.3} />
                      <Text color={termMuted} fontFamily="mono" fontSize="2xs">{group.items.length} {t('projects.projects')}</Text>
                    </HStack>
                    <Box position="relative">
                      <Box bg={termBorder} bottom="12px" left="7px" opacity={0.3} position="absolute" top="12px" w="1px" />
                      <VStack align="stretch" gap={0}>
                        {group.items.map((item, idx) => (
                          <FlowNode ct={themes[item.category]} hlc={hlc} isDark={isDark} isLast={idx === group.items.length - 1} item={item} key={item.id} onImageClick={onImgClick} termBorder={termBorder} termMuted={termMuted} termSecondary={termSecondary} termText={termText} />
                        ))}
                      </VStack>
                    </Box>
                  </Box>
          ...
          <Flex align="center" bg={termHeader} borderTop={`1px solid ${termBorder}`} color={termMuted} flexWrap="wrap" fontSize="2xs" gap={2} justify="space-between" px={4} py={1.5}>
            <HStack gap={3}>
            ...
            <HStack gap={1}>
              ...
              <Box bg={termPrompt} css={{ animation: `${blink} 1s step-end infinite` }} h="11px" w="6px" />
            </HStack>
          </Flex>
        </Box>

        {imgPreview && (
          <Dialog.Root onOpenChange={(e) => { if (!e.open) setImgOpen(false) }} open={isImgOpen}>
            ...
            <Dialog.Positioner alignItems="center" display="flex" inset={0} justifyContent="center" position="fixed" zIndex={1400}>
              <Dialog.Content bg="transparent" boxShadow="none" p={0}>
                ...
                <Dialog.Body alignItems="center" display="flex" justifyContent="center" p={0}>
                  <Image alt={imgPreview.alt} bg={isDark ? 'rgba(0,0,0,0.85)' : 'white'} border={`1px solid ${termBorder}`} borderRadius="md" maxH="80vh" maxW="90vw" objectFit="contain" p={4} src={imgPreview.src} />
                </Dialog.Body>
              </Dialog.Content>
            </Dialog.Positioner>
          </Dialog.Root>
